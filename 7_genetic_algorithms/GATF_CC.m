...
% COMPETING CONVENTION TABLE
T = table(rand(2,1),rand(2,1),'VariableNames',{'NUM','DEN'});
T(:,:)=[];
...
for i=1:ngen
    ...
    for j=1:((popsize/2)-1)  % -1 to adjust for elitism
        ...
        % TOURNAMENT
        k = 0.7;
        chr1 = tournamentk(fitvec,k); % attenzione che è un indice
        chr2 = tournamentk(fitvec,k); % attenzione che è un indice
        ...
        if rand(1) <= pcross
            % newchrom =  tsp_cross(pop(:,chr1),pop(:,chr2));
            newchrom =  twop_cross(pop(:,chr1),pop(:,chr2));

        else 
            % se non faccio cross over rimangono i genitori
            newchrom = [pop(:,chr1) pop(:,chr2)];   
        end 
        
        % --------------------------------------------------------- %
        % MUTAZIONE
        newchrom = tsf_mut(newchrom,pmut);
        
        % aggiornamento della popolazione
        newpop = [newpop newchrom];
        newpop = checkerCC(newpop);
...
function pop = tsf_pop(popsize,npar)
        ...
        for i=1:popsize
            ...
            while(flag ~= 0)
                ...
                DEN_ROUNDED = round(den);

                position = size(cellArray,1)+1;
            

                fun = @(A,B)cellfun(@isequal,A,B);

                idx = find(cellfun(@(c)all(fun({string(NUM_ROUNDED(:,:)) string(DEN_ROUNDED(:,:))},c)),num2cell(cellArray,2)));
                
                if(length(idx) == 0)
                    cellArray{position ,1} = string(NUM_ROUNDED);
                    cellArray{position ,2} = string(DEN_ROUNDED);
                    flag = 0;
                else
                    disp("happened");
                end
...
function npop = checkerCC(pop)
        ...
        for i=1:popsize
            ...
            while(1)
                ...
                idx = find(cellfun(@(c)all(fun({string(NUM_ROUNDED(:,:)) string(DEN_ROUNDED(:,:))},c)),num2cell(cellArray,2)), 1);
                if(isempty(idx))
                    cellArray{position ,1} = string(NUM_ROUNDED);
                    cellArray{position ,2} = string(DEN_ROUNDED);
                    break;
